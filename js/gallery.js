...
import { vertexShader, fragmentShader } from "./gallery-shaders.js";

// Use first 25 images only
const GRID_IMAGE_COUNT = 25;
const gridImages = galleryImages.slice(0, GRID_IMAGE_COUNT);

const config = {
    cellSize: 0.75,
    zoomLevel: 1.25,
    lerpFactor: 0.075,
    borderColor: "rgba(242, 238, 218, 0.15)",
    backgroundColor: "rgba(20, 20, 20, 1)",
    textColor: "rgba(140, 138, 127, 1)",
    hoverColor: "rgba(238, 100, 54, 0.1)",
};
...
const createTextTexture = (title, year) => {
    ...
    ctx.font = "40px 'Geist Mono', monospace";
    ctx.fillStyle = config.textColor;
    ctx.textBaseline = "middle";

    ctx.textAlign = "left";
    ctx.fillText(title.toUpperCase(), 15, 64);
    ctx.textAlign = "right";
    ctx.fillText(year.toString().toUpperCase(), canvas.width - 15, 64);
...
const loadTextures = () => {
    ...
    return new Promise((resolve) => {
        gridImages.forEach((project) => {
            const texture = textureLoader.load(project.image, () => {
                if (++loadedCount === gridImages.length) resolve(imageTextures);
            });
...
const init = async () => {
    ...
    const uniforms = {
        ...
        uTextureCount: { value: gridImages.length },
